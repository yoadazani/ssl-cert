...
```
4. Encode the extfile to utf8
```bash
Get-Content extfile.cnf | Out-File -Encoding utf8 extfile-utf8.cnf
```
5. Create the certificate
```bash
openssl x509 -req -sha256 -days 365 -in cert.csr -CA ca.pem -CAkey ca-key.pem -out cert.pem -extfile extfile.cnf -CAcreateserial
```
6. Merge the files into one file
```bash
cat cert.pem > fullchain.pem
cat ca.pem >> ./fullchain.pem
```
After you dune the sertifcate key is the cert-key.pem and the certificate file is the fullchain.pem
